...
type ChargeRow = {
  ...
  markupBasic: string
  markupSC: string
  markupBasicSC: string
  createdDate: string
  updatedDate: string
}

const initialRows: ChargeRow[] = [
  { id: 1, trip: "M", trainClass: "ALL", fareType: "ALL", groupType: "ALL", agentId: "ALL", origin: "ALL", destination: "ALL", markupType: "F", amount: "150.0000", markupBasic: "0.00", markupSC: "0.00", markupBasicSC: "0.00", createdDate: "2/11/2026 10:45:37 AM", updatedDate: "-" },
  { id: 2, trip: "P", trainClass: "ALL", fareType: "Premium", groupType: "ALL", agentId: "ALL", origin: "ALL", destination: "ALL", markupType: "F", amount: "250.0000", markupBasic: "0.00", markupSC: "0.00", markupBasicSC: "0.00", createdDate: "8/6/2024 8:49:30 PM", updatedDate: "-" },
  { id: 3, trip: "M", trainClass: "ALL", fareType: "Premium", groupType: "ALL", agentId: "ALL", origin: "ALL", destination: "ALL", markupType: "F", amount: "75.0000", markupBasic: "0.00", markupSC: "0.00", markupBasicSC: "0.00", createdDate: "4/16/2023 1:06:23 PM", updatedDate: "-" },
]

const emptyForm = {
  tripType: "Mail/Express",
  trainClass: "",
  fareType: "ALL",
  groupType: "",
  agentId: "ALL",
...
export default function RailwaysMiscServiceChargePage() {
  ...
  const handleSave = () => {
    if (!form.trainClass || !form.groupType || !form.markupType) return
    ...
    setConfirmAction(() => () => {
      ...
      if (isEdit) {
        setRows((prev) => prev.map((r) => r.id === editingId ? {
          ...r,
          trip: form.tripType === "Mail/Express" ? "M" : "P",
          ...
          markupSC: Number(form.markupSC || 0).toFixed(2),
          markupBasicSC: Number(form.markupBasicSC || 0).toFixed(2),
          updatedDate: now,
        } : r))
      } else {
        setRows((prev) => [
  ...
  const editRow = (r: ChargeRow) => {
    setEditingId(r.id)
    setForm({
      tripType: r.trip === "M" ? "Mail/Express" : "Rajdhani/Shatabdi/Duronto",
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="grid grid-cols-1 gap-x-6 gap-y-4 px-6 py-5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            <label className={labelCls}>Markup SC</label>
            <input value={form.markupSC} onChange={(e) => update("markupSC", e.target.value.replace(/[^0-9.]/g, ""))} className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Markup (Basic+SC)</label>
            <input value={form.markupBasicSC} onChange={(e) => update("markupBasicSC", e.target.value.replace(/[^0-9.]/g, ""))} className={inputCls} />
          </div>
          <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-800 text-xs text-white">
                ...
                <th className="px-4 py-3 text-left font-medium whitespace-nowrap">MarkupOnSC</th>
                <th className="px-4 py-3 text-left font-medium whitespace-nowrap">MarkupOnBasicSC</th>
