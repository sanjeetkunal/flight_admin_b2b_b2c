...
type ChargeRow = {
  ...
  markupBasic: string
  markupTax: string
  markupBasicTax: string
  createdDate: string
  updatedDate: string
}

const initialRows: ChargeRow[] = [
  { id: 1, trip: "D", category: "ALL", packageType: "ALL", groupType: "ALL", agentId: "ALL", origin: "ALL", destination: "ALL", markupType: "F", amount: "500.0000", markupBasic: "0.00", markupTax: "0.00", markupBasicTax: "0.00", createdDate: "2/11/2026 10:45:37 AM", updatedDate: "-" },
  { id: 2, trip: "I", category: "International", packageType: "ALL", groupType: "ALL", agentId: "ALL", origin: "ALL", destination: "ALL", markupType: "F", amount: "1500.0000", markupBasic: "0.00", markupTax: "0.00", markupBasicTax: "0.00", createdDate: "8/6/2024 8:49:30 PM", updatedDate: "-" },
  { id: 3, trip: "D", category: "Beach", packageType: "ALL", groupType: "ALL", agentId: "ALL", origin: "ALL", destination: "ALL", markupType: "F", amount: "350.0000", markupBasic: "0.00", markupTax: "0.00", markupBasicTax: "0.00", createdDate: "4/16/2023 1:06:23 PM", updatedDate: "-" },
]
...
export default function HolidaysMiscServiceChargePage() {
  ...
  const handleSave = () => {
    if (!form.category || !form.groupType || !form.markupType) return
    ...
    setConfirmAction(() => () => {
      ...
      if (isEdit) {
        setRows((prev) => prev.map((r) => r.id === editingId ? {
          ...
          markupTax: Number(form.markupTax || 0).toFixed(2),
          markupBasicTax: Number(form.markupBasicTax || 0).toFixed(2),
          updatedDate: now,
        } : r))
      } else {
        setRows((prev) => [
  ...
  const editRow = (r: ChargeRow) => {
    ...
    setForm({
      ...
      markupTax: r.markupTax,
      markupBasicTax: r.markupBasicTax,
      amount: r.amount,
    })
  ...
  return (
    <div className="space-y-4">
      {/* Header bar */}
      <div className="rounded-xl bg-pink-500 px-6 py-3 shadow-sm">
        <p className="text-sm font-medium text-white">Holiday Setting <span className="opacity-70">&gt;</span> MISC SERVICE CHARGE</p>
      </div>
      ...
      <div className="rounded-xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="grid grid-cols-1 gap-x-6 gap-y-4 px-6 py-5 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            <label className={labelCls}>Markup Tax</label>
            <input value={form.markupTax} onChange={(e) => update("markupTax", e.target.value.replace(/[^0-9.]/g, ""))} className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Markup (Basic+Tax)</label>
            <input value={form.markupBasicTax} onChange={(e) => update("markupBasicTax", e.target.value.replace(/[^0-9.]/g, ""))} className={inputCls} />
          </div>
          <div>
        ...
        <div className="flex items-center gap-2 border-t border-slate-100 px-6 py-4 dark:border-slate-800">
          <button onClick={handleSave} className="rounded-lg bg-pink-500 px-4 py-2 text-xs font-medium text-white hover:bg-pink-600 transition-colors">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 rounded-lg border border-pink-100 bg-pink-50 px-3 py-2">
          ...
          <span className="text-xs text-pink-600/70">Service Charges</span>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-800 text-xs text-white">
                ...
                <th className="px-4 py-3 text-left font-medium whitespace-nowrap">PackageType</th>
                ...
                <th className="px-4 py-3 text-left font-medium whitespace-nowrap">MarkupOnTax</th>
                <th className="px-4 py-3 text-left font-medium whitespace-nowrap">MarkupOnBasicTax</th>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-slate-50/60 transition-colors dark:hover:bg-slate-800/60">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap font-mono text-xs text-pink-700 dark:text-pink-400">{r.markupType}</td>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-slate-600 dark:text-slate-300">{r.markupTax}</td>
                  <td className="px-4 py-3 whitespace-nowrap text-slate-600 dark:text-slate-300">{r.markupBasicTax}</td>
