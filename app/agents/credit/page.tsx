    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 rounded-xl border border-slate-100 bg-white px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-between dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 dark:border-slate-700 dark:bg-slate-800">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search agent name or ID..." className="w-40 bg-transparent text-xs text-slate-600 placeholder-slate-400 outline-none sm:w-56 dark:text-slate-200" />
        </div>
      </div>
              ...
              {filtered.map((a) => {
                ...
                return (
                  <tr key={a.id} className="hover:bg-slate-50/60 transition-colors dark:hover:bg-slate-800/60 align-top">
                    ...
                    <td className="px-6 py-3 font-semibold text-slate-800 dark:text-slate-100">{formatInr(a.currentLimit)}</td>
                    <td className="px-6 py-3 text-slate-600 dark:text-slate-300">{formatInr(a.used)}</td>
                    <td className="px-6 py-3">
                      <span className={`font-semibold ${balance < a.currentLimit * 0.1 ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}>{formatInr(balance)}</span>
                    </td>
                    <td className="px-6 py-3">
                      <input
                        value={drafts[a.id] || ""}
