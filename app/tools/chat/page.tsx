...
function Avatar({ initial, online }: { initial: string; online?: boolean }) {
  return (
    <div className="relative flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-neutral-700 text-xs font-semibold text-white">
      {initial}
      {online && (
  ...
  return (
    <div
      className={`flex items-start gap-3 px-4 py-3 cursor-pointer ${
        conv.active ? "bg-neutral-900" : "hover:bg-neutral-900/60"
      }`}
    >
      <Avatar initial={conv.initial} online={conv.online} />
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <p className="truncate text-sm font-medium text-neutral-100">{conv.name}</p>
          <span className="flex-shrink-0 text-[11px] text-neutral-500">{conv.time}</span>
        </div>
        <p className="truncate text-sm text-neutral-300">{conv.subject}</p>
        <p className="truncate text-xs text-neutral-500">{conv.preview}</p>
      </div>
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] min-h-[640px] overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-950 text-neutral-100 shadow-xl">
      ...
      <section className="flex w-full flex-shrink-0 flex-col border-r border-neutral-800/80 md:w-[380px]">
        ...
        <div className="flex items-center gap-5 border-b border-neutral-800/80 px-4 pt-3 text-sm">
          <button className="border-b-2 border-neutral-100 pb-2.5 font-medium text-neutral-100">
            All <span className="text-neutral-500">(24)</span>
          </button>
          <button className="pb-2.5 text-neutral-500 hover:text-neutral-300">
            Open <span className="text-neutral-600">(18)</span>
          </button>
          <button className="pb-2.5 text-neutral-500 hover:text-neutral-300">
            Snoozed <span className="text-neutral-600">(2)</span>
          </button>
          <button className="pb-2.5 text-neutral-500 hover:text-neutral-300">Closed</button>
        </div>

        <div className="flex-1 overflow-y-auto">
          <div className="flex items-center gap-1.5 px-4 pb-1 pt-3 text-xs font-medium text-neutral-500">
      ...
      <section className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-center justify-between border-b border-neutral-800/80 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-neutral-100">Olivia Rhye</p>
              <p className="text-xs text-neutral-500">Senior DevOps Engineer</p>
            </div>
          </div>
            ...
            <div key={i} className={`flex items-end gap-2.5 ${m.from === "me" ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`flex max-w-[70%] flex-col ${m.from === "me" ? "items-end" : "items-start"}`}>
                <div className="relative">
                  ...
                  {m.reaction && (
                    <span
                      className={`absolute -bottom-2.5 flex h-6 w-6 items-center justify-center rounded-full border-2 border-neutral-950 bg-neutral-800 text-xs ${
                ...
                <span className="mt-3 text-[11px] text-neutral-500">{m.time}</span>
              </div>
            </div>
          ))}
        </div>

        <div className="border-t border-neutral-800/80 px-6 pt-3">
          <div className="flex items-center gap-5 text-sm">
            <button className="border-b-2 border-neutral-100 pb-2.5 font-medium text-neutral-100">
              Reply
            </button>
            <button className="pb-2.5 text-neutral-500 hover:text-neutral-300">Internal note</button>
          </div>

          <div className="py-3">
            <p className="text-sm text-neutral-500">Type your message...</p>
          </div>

          <div className="flex items-center justify-between border-t border-neutral-800/80 pb-4 pt-3">
            <div className="flex items-center gap-1 text-neutral-500">
              <IconBtn>
              ...
              <button className="flex h-8 w-8 items-center justify-center rounded-full bg-neutral-100 text-neutral-900">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2l2.4 6.6L21 11l-6.6 2.4L12 20l-2.4-6.6L3 11l6.6-2.4z" />
                </svg>
              </button>
            </div>
            <button className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-indigo-600 text-white hover:bg-indigo-500">
              <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor">
