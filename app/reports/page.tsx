  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        {/* Revenue chart */}
        <div className="xl:col-span-2 rounded-xl border border-slate-100 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              return (
                <div key={m.label}>
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400">₹{(m.revenue / 100000).toFixed(1)}L ({pct}%)</span>
                  </div>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {moduleStats.map((m) => (
                  <tr key={m.label} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-6 py-3 font-semibold text-slate-700 dark:text-slate-200">{m.bookings}</td>
                    <td className="px-6 py-3 font-semibold text-slate-700 dark:text-slate-200">₹{(m.revenue / 100000).toFixed(1)}L</td>
                    <td className="px-6 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400">{m.pct}%</span>
                      </div>
                    </td>
                  </tr>
                ))}
            ...
            {topAgents.map((a, i) => (
              <div key={a.name} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/60">
                ...
                <div className="text-right flex-shrink-0">
                  <p className="font-semibold text-slate-800 dark:text-slate-100">{a.revenue}</p>
        ...
        <div className="flex flex-wrap gap-3">
          {[
            ...
            <button key={r.label} className="flex items-center gap-2 rounded-lg border border-slate-200 px-4 py-2.5 text-xs font-medium text-slate-600 hover:border-indigo-300 hover:bg-indigo-50 hover:text-indigo-700 transition-all dark:border-slate-700 dark:text-slate-300 dark:hover:border-indigo-500/40 dark:hover:bg-indigo-500/10 dark:hover:text-indigo-400">
              ...
              {r.label}
              <span className="rounded bg-slate-100 px-1.5 py-0.5 text-slate-500 dark:bg-slate-800 dark:text-slate-400">{r.fmt}</span>
            </button>
          ))}
        </div>
      </div>
