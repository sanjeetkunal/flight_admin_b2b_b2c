...
type ModuleDef = { key: string; label: string; badge: string }

const MODULES: ModuleDef[] = [
  { key: "flights", label: "Flights", badge: "bg-blue-100 text-blue-700" },
  { key: "railway", label: "Railway", badge: "bg-green-100 text-green-700" },
  { key: "bus", label: "Bus", badge: "bg-orange-100 text-orange-700" },
  { key: "hotel", label: "Hotel", badge: "bg-purple-100 text-purple-700" },
  { key: "holiday", label: "Holiday", badge: "bg-pink-100 text-pink-700" },
  { key: "utility", label: "Utility", badge: "bg-teal-100 text-teal-700" },
  { key: "reports", label: "Reports", badge: "bg-slate-100 text-slate-700" },
  { key: "agents", label: "B2B Agents", badge: "bg-indigo-100 text-indigo-700" },
]
...
const allTrue = () => Object.fromEntries(MODULES.map((m) => [m.key, true]))

const initialRoles: Role[] = [
  { id: "super-admin", name: "Super Admin", badge: "bg-indigo-100 text-indigo-700", dot: "bg-indigo-500", protected: true, permissions: allTrue() },
  { id: "operations", name: "Operations", badge: "bg-blue-100 text-blue-700", dot: "bg-blue-500", permissions: { flights: true, railway: true, bus: true, hotel: false, holiday: false, utility: false, reports: false, agents: false } },
  { id: "sales", name: "Sales", badge: "bg-pink-100 text-pink-700", dot: "bg-pink-500", permissions: { flights: false, railway: false, bus: false, hotel: true, holiday: true, utility: false, reports: false, agents: false } },
  { id: "accounts", name: "Accounts", badge: "bg-teal-100 text-teal-700", dot: "bg-teal-500", permissions: { flights: false, railway: false, bus: false, hotel: false, holiday: false, utility: true, reports: true, agents: false } },
]
...
const inputCls = "w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-indigo-100 focus:border-indigo-300"
...
const roleColorPresets = [
  ...
  { badge: "bg-pink-100 text-pink-700", dot: "bg-pink-500" },
...
function emptyRoleForm(): Omit<Role, "id"> {
  return { name: "", badge: roleColorPresets[1].badge, dot: roleColorPresets[1].dot, permissions: Object.fromEntries(MODULES.map((m) => [m.key, false])) }
}
...
export default function UsersRolesSettingsPage() {
  ...
  function moduleLabelsFor(role: Role | undefined) {
    if (!role) return []
    return MODULES.filter((m) => role.permissions[m.key]).map((m) => m.label)
  }
  ...
  function togglePermission(roleId: string, moduleKey: string) {
    setRoles((prev) => prev.map((r) => (r.id === roleId && !r.protected ? { ...r, permissions: { ...r.permissions, [moduleKey]: !r.permissions[moduleKey] } } : r)))
  }
  ...
  const stats = [
    ...
    { label: "Full Module Access", value: roles.filter((r) => MODULES.every((m) => r.permissions[m.key])).length, color: "text-violet-600 bg-violet-50 border-violet-100" },
  ]
    ...
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 px-6 py-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs text-slate-700 outline-none focus:border-indigo-300 focus:ring-2 focus:ring-indigo-100 sm:w-56"
            />
              ...
              {filteredUsers.map((u) => {
                ...
                return (
                  <tr key={u.id} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-6 py-3">
                      <span className={`rounded-md px-2 py-0.5 text-xs font-medium ${role?.badge ?? "bg-slate-100 text-slate-600"}`}>{role?.name ?? "—"}</span>
                    </td>
                    ...
                    <td className="px-6 py-3 text-xs text-slate-500">{u.lastLogin}</td>
                    <td className="px-6 py-3">
                      <button
                        onClick={() => toggleUserStatus(u)}
                        className={`rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors ${u.status === "Active" ? "bg-emerald-100 text-emerald-700 hover:bg-emerald-200" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}
                      >
                        {u.status}
                      </button>
                    </td>
      ...
      <div className="rounded-xl border border-slate-100 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800">
          ...
          <button onClick={openAddRole} className="flex flex-shrink-0 items-center gap-1.5 rounded-lg border border-indigo-200 px-3 py-2 text-xs font-medium text-indigo-600 hover:bg-indigo-50">
            <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
            Add Role
          </button>
        </div>
                ...
                <tr key={r.id} className="hover:bg-slate-50/50">
                  ...
                  {MODULES.map((m) => (
                    <td key={m.key} className="px-3 py-3 text-center">
                      <input
                        type="checkbox"
                        checked={r.permissions[m.key]}
                        disabled={r.protected}
                        onChange={() => togglePermission(r.id, m.key)}
                        className="h-4 w-4 rounded border-slate-300 text-indigo-600 accent-indigo-600 disabled:opacity-50"
                      />
                    </td>
                  ))}
                  <td className="px-6 py-3 text-xs text-slate-500">{users.filter((u) => u.roleId === r.id).length}</td>
                  <td className="px-6 py-3 text-right">
                    {!r.protected && <button onClick={() => removeRole(r)} className="text-xs text-red-400 hover:text-red-600">Remove</button>}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4" onClick={closeUserModal}>
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
              <button onClick={closeUserModal} className="rounded-lg border border-slate-200 px-3.5 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50">Cancel</button>
      ...
      {/* Add Role modal */}
      {showAddRole && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 px-4" onClick={() => setShowAddRole(false)}>
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-start justify-between border-b border-slate-100 px-6 py-4">
              <div>
                <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Add Role</h3>
                <p className="text-xs text-slate-400">Define a name, color and module access for the new role</p>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {MODULES.map((m) => (
                    <button
                      key={m.key}
                      type="button"
                      onClick={() => setRoleForm({ ...roleForm, permissions: { ...roleForm.permissions, [m.key]: !roleForm.permissions[m.key] } })}
                      className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition-all ${roleForm.permissions[m.key] ? "border-indigo-200 bg-indigo-50 text-indigo-700" : "border-slate-200 text-slate-500 hover:bg-slate-50"}`}
                    >
                      {m.label}
                    </button>
                  ))}
            ...
            <div className="flex items-center justify-end gap-2 border-t border-slate-100 px-6 py-4">
              <button onClick={() => setShowAddRole(false)} className="rounded-lg border border-slate-200 px-3.5 py-2 text-xs font-medium text-slate-600 hover:bg-slate-50">Cancel</button>
              <button onClick={saveRole} className="rounded-lg bg-indigo-600 px-3.5 py-2 text-xs font-medium text-white hover:bg-indigo-700 transition-colors">Create Role</button>
            </div>
          </div>
        </div>
      )}
