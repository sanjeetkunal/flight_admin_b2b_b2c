  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
        {tabs.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 rounded-lg px-3 py-2 text-xs font-medium transition-all ${activeTab === tab ? "bg-indigo-600 text-white shadow-sm" : "text-slate-500 hover:bg-slate-50 hover:text-slate-700"}`}>
            {tab}
          </button>
        ))}
      </div>
          ...
          <div className="rounded-xl border border-slate-100 bg-white p-6 shadow-sm space-y-5">
            ...
            {[
              ...
              <div key={f.label}>
                ...
                <input defaultValue={f.value} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm text-slate-800 outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100" />
              </div>
            ))}
          </div>
          <div className="space-y-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                  {[
                    { tier: "Platinum", values: [1.5, 1.0, 2.0, 3.5, 6.0, 0.5], color: "bg-violet-100 text-violet-700" },
                    { tier: "Gold", values: [2.0, 1.5, 2.5, 4.0, 7.0, 0.8], color: "bg-yellow-100 text-yellow-700" },
                    { tier: "Silver", values: [2.5, 1.5, 3.0, 5.0, 8.0, 1.0], color: "bg-slate-100 text-slate-600" },
                    { tier: "Bronze", values: [3.0, 2.0, 3.5, 6.0, 9.0, 1.2], color: "bg-orange-100 text-orange-700" },
                  ].map((row) => (
                    <tr key={row.tier} className="hover:bg-slate-50/50">
                      ...
                      {row.values.map((v, i) => (
                        <td key={i} className="py-3 pr-4">
                          <input defaultValue={v.toFixed(1)} className="w-16 rounded border border-slate-200 px-2 py-1 text-xs text-slate-700 outline-none focus:border-indigo-300" />
                        </td>
                      ))}
                    </tr>
                  ))}
        ...
        <div className="rounded-xl border border-slate-100 bg-white p-6 shadow-sm">
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={item.label} className="flex items-center justify-between rounded-lg border border-slate-100 p-4 hover:bg-slate-50">
      ...
      {activeTab === "API & Integration" && (
        <div className="space-y-5">
          {[
            ...
            <div key={api.name} className="flex items-center justify-between rounded-xl border border-slate-100 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-4">
                <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${api.bg}`}>
                  <svg className={`h-5 w-5 ${api.color}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" />
                  </svg>
                </div>
                <div>
                  <p className="font-medium text-slate-800">{api.name}</p>
                  <p className="font-mono text-xs text-slate-400">{api.key}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${api.status === "Connected" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{api.status}</span>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
              {[
                ...
                <tr key={u.email} className="hover:bg-slate-50/60">
                  ...
                  <td className="px-6 py-3">
                    <span className={`rounded-md px-2 py-0.5 text-xs font-medium ${u.role === "Super Admin" ? "bg-indigo-100 text-indigo-700" : "bg-slate-100 text-slate-600"}`}>{u.role}</span>
                  </td>
                  <td className="px-6 py-3 text-xs text-slate-600">{u.modules}</td>
                  <td className="px-6 py-3 text-xs text-slate-500">{u.lastLogin}</td>
                  <td className="px-6 py-3">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${u.status === "Active" ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{u.status}</span>
                  </td>
                  <td className="px-6 py-3">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {[
                  ...
                  <tr key={r.inv} className="hover:bg-slate-50/60">
                    <td className="py-3 pr-6 font-mono text-xs text-slate-700">{r.inv}</td>
                    <td className="py-3 pr-6 text-slate-700">{r.period}</td>
                    <td className="py-3 pr-6 font-semibold text-slate-800">{r.amount}</td>
                    <td className="py-3 pr-6"><span className="rounded-full bg-emerald-100 px-2.5 py-0.5 text-xs font-medium text-emerald-700">{r.status}</span></td>
                    <td className="py-3">
